...
-- 11. LIST EMPLOYEES FROM RESEARCH & ACCOUNTING HAVING ATLEAST 2 REPORTING. 							-----------IMPORTANT--------(**********)
	SELECT ENAME
    FROM EMP
    WHERE DEPTNO IN (SELECT DEPTNO
						FROM DEPT
...
--                         WHERE DNAME IN ('RESEARCH','ACCOUNTING'))
-- 	GROUP BY MGR
--     HAVING COUNT(*) > 1; ----------------> this is wrong as we cant pass non-aggregate column in select clause 
--  											which is not the part of GROUP BY clause.
...
-- 26. DISPLAY EMPLOYEE DETAILS WHO ARE REPORTING TO BLAKE AND HAVE COMMISSION WITHOUT USING NULL OR NOT NULL
	...
	SELECT E1.*
    FROM EMP E1,EMP E2
    WHERE E1.MGR = E2.EMPNO AND E2.ENAME = 'BLAKE' AND E1.COMM > 0;
...
-- 30. LIST THE EMPLOYEES WHO JOINED AFTER 2 YEARS OF FIRST EMPLOYEE OF THE COMPANY AND MORE THAN BLAKE SALARY  ---------- (*)
    ...
    WHERE SAL > ALL(SELECT SAL
					FROM EMP
                    WHERE ENAME = 'BLAKE') AND 'HIREDATE' > (SELECT MIN('HIREDATE') + INTERVAL '2' YEAR
															FROM EMP);
...
-- 32. LIST ALL THE EMPLOYEES WHOSE JOB IS SAME AS JONES AND THEIR SALARY LESSER THAN SCOTT
    ...
    WHERE JOB IN (SELECT JOB
					FROM EMP
                    WHERE ENAME = 'JONES') AND SAL< ALL(SELECT SAL 
														FROM EMP
                                                        WHERE ENAME = 'SCOTT');
-- 33.DISPLAY ALL THE EMPLOYEES OF DEPARTMENT 30, 20 WITH THERE ANUAL SALARY AND HAVING ATLEAST 3 EMPLOYEES
    ...
    WHERE DEPTNO IN (SELECT DEPTNO
					FROM EMP
                    GROUP BY DEPTNO
                    HAVING COUNT(*) >2) AND DEPTNO IN (30,20);
...
-- 37.LIST EMPLOYEES WHO JOINED AFTER 4 YEARS OF 1ST EMPLOYEE OF THE COMPANY AND LESS THAN BLAKE SALARY.
	SELECT ENAME
    FROM EMP
    WHERE HIREDATE > DATE_ADD((SELECT MIN(HIREDATE)
										FROM EMP),INTERVAL 4 YEAR) AND SAL< ( SELECT SAL
																				FROM EMP
                                                                                WHERE ENAME = 'BLAKE');
-- 38.DISPLAY THE DEPARTMENT INFORMATION OF EMPLOYEE WHO IS WORKING FOR NEW YORK LOCATION
	SELECT *
...
-- 56.DISPLAY THE NUMBER OF EMPLOYEES WHO WORK FOR RESEARCH DEPT AND THEIR SALARY IS LESSER THAN ONE OF THE SALARY IN DEPARTMENT IN 10.
    ...
    WHERE DEPTNO IN(SELECT DEPTNO
					FROM DEPT
                    WHERE DNAME = 'RESEARCH') AND SAL < ALL(SELECT SAL
															FROM EMP
                                                            WHERE DEPTNO = 10);
